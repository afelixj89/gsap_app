...
var _harness = (targets) => {
  let target = targets[0], harnessPlugin, i;
...
var _arrayContainsAny = (toSearch, toFind) => {
  let l = toFind.length, i = 0;
  for (; toSearch.indexOf(toFind[i]) < 0 && ++i < l; ) {
  }
  return i < l;
};
var _lazyRender = () => {
  let l = _lazyTweens.length, a = _lazyTweens.slice(0), i, tween;
  _lazyLookup = {};
...
var _arraysMatch = (a1, a2) => {
  let i = a1.length, match = i === a2.length;
...
var _parsePosition = (animation, position, percentAnimation) => {
  let labels = animation.labels, recent = animation._recent || _zeroPosition, clippedDuration = animation.duration() >= _bigNum ? recent.endTime(false) : animation._dur, i, offset, isPercent;
...
var _roundModifier = (v) => {
  let p = Math.pow(10, ((v + "").split(".")[1] || "").length);
  return (raw) => {
...
var snap = (snapTo, value) => {
  ...
  return _conditionalReturn(value, !isArray ? _roundModifier(snapTo) : _isFunction(snapTo) ? (raw) => {
    ...
    let x = parseFloat(is2D ? raw.x : raw), y = parseFloat(is2D ? raw.y : 0), min = _bigNum, closest = 0, i = snapTo.length, dx, dy;
    while (i--) {
      if (is2D) {
        dx = snapTo[i].x - x;
        dy = snapTo[i].y - y;
...
var _replaceRandom = (value) => {
  let prev = 0, s = "", i, nums, end, isArray;
...
var interpolate = (start, end, progress, mutate) => {
  ...
  if (!func) {
    let isString = _isString(start), master = {}, p, i, interpolators, l, il;
    ...
    if (!interpolators) {
      ...
      func = (p2) => _renderPropTweens(p2, master) || (isString ? start.p : start);
    }
  }
  return _conditionalReturn(progress, func);
};
var _getLabelInDirection = (timeline, fromTime, backward) => {
  let labels = timeline.labels, min = _bigNum, p, distance, label;
  for (p in labels) {
...
var _formatColors = (s, toHSL, orderMatchData) => {
  let result = "", colors = (s + result).match(_colorExp), type = toHSL ? "hsla(" : "rgba(", i = 0, c, shell, d, l;
  if (!colors) {
    return s;
  }
...
var _colorExp = function() {
  let s = "(?:\\b(?:(?:rgb|rgba|hsl|hsla)\\(.+?\\))|\\B#(?:[0-9a-f]{3,4}){1,2}\\b", p;
...
var _ticker = function() {
  let _getTime = Date.now, _lagThreshold = 500, _adjustedLag = 33, _startTime = _getTime(), _lastUpdate = _startTime, _gap = 1e3 / 240, _nextTime = _gap, _listeners2 = [], _id, _req, _raf, _self, _delta, _i, _tick = (v) => {
    let elapsed = _getTime() - _lastUpdate, manual = v === true, overlap, dispatch, time, frame;
...
var _parseObjectInString = (value) => {
  let obj = {}, split = value.substr(1, value.length - 3).split(":"), key = split[0], i = 1, l = split.length, index, val, parsedVal;
  for (; i < l; i++) {
    val = split[i];
...
var _invertEase = (ease) => (p) => 1 - ease(1 - p);
...
var _insertEase = (names, easeIn, easeOut = (p) => 1 - easeIn(1 - p), easeInOut = (p) => p < 0.5 ? easeIn(p * 2) / 2 : 1 - easeIn((1 - p) * 2) / 2) => {
...
var _easeInOutFromOut = (easeOut) => (p) => p < 0.5 ? (1 - easeOut(1 - p * 2)) / 2 : 0.5 + easeOut((p - 0.5) * 2) / 2;
var _configElastic = (type, amplitude, period) => {
  let p1 = amplitude >= 1 ? amplitude : 1, p2 = (period || (type ? 0.3 : 0.45)) / (amplitude < 1 ? amplitude : 1), p3 = p2 / _2PI * (Math.asin(1 / p1) || 0), easeOut = (p) => p === 1 ? 1 : p1 * 2 ** (-10 * p) * _sin((p - p3) * p2) + 1, ease = type === "out" ? easeOut : type === "in" ? (p) => 1 - easeOut(1 - p) : _easeInOutFromOut(easeOut);
  p2 = _2PI / p2;
  ease.config = (amplitude2, period2) => _configElastic(type, amplitude2, period2);
  return ease;
};
var _configBack = (type, overshoot = 1.70158) => {
  let easeOut = (p) => p ? --p * p * ((overshoot + 1) * p + overshoot) + 1 : 0, ease = type === "out" ? easeOut : type === "in" ? (p) => 1 - easeOut(1 - p) : _easeInOutFromOut(easeOut);
...
((n, c) => {
  let n1 = 1 / c, n2 = 2 * n1, n3 = 2.5 * n1, easeOut = (p) => p < n1 ? n * p * p : p < n2 ? n * (p - 1.5 / c) ** 2 + 0.75 : p < n3 ? n * (p -= 2.25 / c) * p + 0.9375 : n * (p - 2.625 / c) ** 2 + 0.984375;
...
var Timeline = class extends Animation {
  ...
  getById(id) {
    let animations = this.getChildren(1, 1, 1), i = animations.length;
    while (i--) {
  ...
  killTweensOf(targets, props, onlyActive) {
    let tweens = this.getTweensOf(targets, onlyActive), i = tweens.length;
    while (i--) {
  ...
  shiftChildren(amount, adjustLabels, ignoreBeforeTime = 0) {
    let child = this._first, labels = this.labels, p;
    while (child) {
      if (child._start >= ignoreBeforeTime) {
        child._start += amount;
        child._end += amount;
...
var _updatePropTweens = (tween, property, value, start, startIsRelative, ratio, time, skipRecursion) => {
  let ptCache = (tween._pt && tween._ptCache || (tween._ptCache = {}))[property], pt, rootPT, lookup, i;
  if (!ptCache) {
...
var _addAliasesToVars = (targets, vars) => {
  let harness = targets[0] ? _getCache(targets[0]).harness : 0, propertyAliases = harness && harness.aliases, copy, p, i, aliases;
  if (!propertyAliases) {
    return vars;
  }
  copy = _merge({}, vars);
...
var _parseKeyframe = (prop, obj, allProps, easeEach) => {
  let ease = obj.ease || easeEach || "power1.inOut", p, a;
  if (_isArray(obj)) {
...
var _parseFuncOrString = (value, tween, i, target, targets) => _isFunction(value) ? value.call(tween, i, target, targets) : _isString(value) && ~value.indexOf("random(") ? _replaceRandom(value) : value;
...
var Tween = class _Tween extends Animation {
  ...
  kill(targets, vars = "all") {
    ...
    let parsedTargets = this._targets, killingTargets = targets ? toArray(targets) : parsedTargets, propTweenLookup = this._ptLookup, firstPT = this._pt, overwrittenProps, curLookup, curOverwriteProps, props, p, pt, i;
...
var _onMediaChange = () => {
  ...
  if (time - _lastMediaTime > 2) {
    ...
    _media.forEach((c) => {
      let queries = c.queries, conditions = c.conditions, match, p, anyMatch, toggled;
      for (p in queries) {
...
var Context = class _Context {
  ...
  kill(revert, matchMedia) {
    if (revert) {
      let tweens = this.getTweens(), i = this.data.length, t;
      while (i--) {
        t = this.data[i];
        if (t.data === "isFlip") {
          t.revert();
...
var MatchMedia = class {
  ...
  add(conditions, func, scope) {
    ...
    let context = new Context(0, scope || this.scope), cond = context.conditions = {}, mq, p, active;
...
var _gsap = {
  ...
  quickSetter(target, property, unit) {
    ...
    target = target[0] || {};
    let Plugin = _plugins[property], cache = _getCache(target), p = cache.harness && (cache.harness.aliases || {})[property] || property, setter = Plugin ? (value) => {
      let p2 = new Plugin();
      _quickTween._pt = 0;
...
var _addModifiers = (tween, modifiers) => {
  let targets = tween._targets, p, i, pt;
  for (p in modifiers) {
    i = targets.length;
    while (i--) {
...
var _revertStyle = function() {
  let props = this.props, target = this.target, style = target.style, cache = target._gsap, i, p;
...
var _checkPropPrefix = (property, element, preferPrefix) => {
  let e = element || _tempDiv, s = e.style, i = 5;
...
var _tweenComplexCSSString = function(target, prop, start, end) {
  if (!start || start === "none") {
    let p = _checkPropPrefix(prop, target, 1), s = p && _getComputedProperty(target, p, 1);
    if (s && s !== start) {
      prop = p;
      start = s;
...
var _renderClearProps = (ratio, data) => {
  if (data.tween && data.tween._time === data.tween._dur) {
    let target = data.t, style = target.style, props = data.u, cache = target._gsap, prop, clearTransforms, i;
...
var _addRawTransformPTs = (plugin, transforms, target) => {
  let startCache = _assign({}, target._gsap), exclude = "perspective,force3D,transformOrigin,svgOrigin", style = target.style, endCache, p, startValue, endValue, startNum, endNum, startUnit, endUnit;
  if (startCache.svg) {
...
var CSSPlugin = {
  ...
  init(target, vars, tween, index, targets) {
    let props = this._props, style = target.style, startAt = tween.vars.startAt, startValue, endValue, endNum, startNum, type, specialProp, p, startUnit, endUnit, relative, isTransformRelated, transformPropTween, cache, smooth, hasPriority, inlineProps;
    _pluginInitted || _initCore();
...
gsap/src/CSSPlugin.js:
  ...
  *)
*/
//# sourceMappingURL=gsap_src.js.map
